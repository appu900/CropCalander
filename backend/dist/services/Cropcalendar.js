...
class CropCelendarService {
    ...
    async createCropCalendar(requestId, expertId) {
        try {
            const cropCalendar = await client_1.prisma.cropCalandar.findFirst({
                where: {
                    requestId: requestId,
                },
            });
            ...
            const response = await client_1.prisma.cropCalandar.create({
                data: payload,
            });
            return response;
        }
    ...
    async createActivities(cropCalendarId, cropcalendarActivityPayload) {
        try {
            ...
            const cropCalendar = await client_1.prisma.cropCalandar.findUnique({
                where: {
                    id: cropCalendarId,
                },
                select: {
